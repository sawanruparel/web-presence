...
interface AccessModalProps {
  isOpen: boolean
  onClose: () => void
  onSubmit: (payload: AccessPayload) => Promise<void>
  title: string
...
export function AccessModal({ 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (accessMode === 'password' && !password.trim()) return
    if (accessMode === 'email-list' && !email.trim()) return
    
    try {
      await onSubmit({
        password: accessMode === 'password' ? password : undefined,
        email: accessMode === 'email-list' ? email : undefined
      })
      setPassword('')
      setEmail('')
    } catch (err) {
  ...
  const getAccessModeDescription = () => {
    switch (accessMode) {
      case 'password':
        return 'This content is password protected. Please enter the password to continue.'
      case 'email-list':
        return 'This content is restricted to approved emails. Please enter your email address.'
      case 'open':
        return 'This content is publicly accessible.'
      default:
        return 'This content is protected. Please verify your access.'
    }
  }

  // Open content doesn't need a modal
  if (accessMode === 'open') {
    return null
  }
        ...
        <DialogPanel className="mx-auto max-w-sm rounded-lg bg-white p-6 shadow-xl" style={{ backgroundColor: 'var(--color-background)', color: 'var(--color-text)' }}>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex justify-end space-x-3">
              ...
              <button
                type="submit"
                disabled={isLoading || (accessMode === 'password' && !password.trim()) || (accessMode === 'email-list' && !email.trim())}
