...
export function TeachingsPage({ teachings }: TeachingsPageProps) {
  return (
    <main className="mx-auto max-w-2xl px-4 py-12 leading-relaxed" style={{ backgroundColor: 'var(--color-background)', color: 'var(--color-text)' }}>
      <header>
        <h1 className="text-3xl font-semibold" style={{ color: 'var(--color-text)' }}>Teachings</h1>
        <p className="mt-2" style={{ color: 'var(--color-text-muted)' }}>
          Educational content and resources for learning.
        </p>
        <nav className="mt-4 flex gap-4 text-sm" style={{ color: 'var(--color-text-muted)' }}>
          ...
          <a href="/teachings" className="hover:underline font-semibold">Teachings</a>
      ...
      <section className="mt-10">
        <ul className="space-y-6">
          {teachings.map((teaching) => (
            <li key={teaching.slug} className="border-b pb-6" style={{ borderColor: 'var(--color-divider)' }}>
              <a href={`/teachings/${teaching.slug}`} className="block hover:underline">
                <h2 className="text-xl font-medium mb-2" style={{ color: 'var(--color-text)' }}>
                  {teaching.title}
                </h2>
                <p className="text-sm mb-2" style={{ color: 'var(--color-text-muted)' }}>
                  {teaching.date} · {teaching.readTime}
                </p>
